...
public class TextureAssign : MonoBehaviour
{
	...
	bool AssignFolderTexturesToObject(GameObject selectedObject, string folder)
	{
		...
		{ 
			...
			if(material != null)
			{
				textureFound = AssignTexturePathListToObject(material, matIndex, texturePathsList);
			}
		}
	...
	bool AssignTexturePathListToObject(Material material, int matIndex, List<string> texturePathsList)
	{
		bool textureFound = false;
		...
		for (int i = 0; i < texturePathsList.Count; i++)
		{
			...
			if (valid)
			{
                if (material.name.Contains(texturePart))
				{
					...
					AssignTexture(material, matIndex, texture, texturePathsList[i], textureExtension);
	...
	void AssignTexture(Material material, int matIndex, Texture2D texture, string texturePath, string textureExtension)
	{
